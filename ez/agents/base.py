...
class Agent:
        ...
        while not self.is_finished(step_count):
            ...
            if is_main_process and step_count % pb_interval == 0:
                ...
                train_log_str = '[Train] {}, step {}/{}, {:.3f}h left. lr={:.3f}, avg time={:.3f}s, batchQ={}, '\
                              ...
                              ''.format(self.config.env.game, step_count, total_steps, left_time, lr, avg_time,
                                    batch_queue_size, self_play_return, traj_num, transition_num / 1000,
            ...
            if is_main_process and step_count % self.config.log.log_interval == 0:
                ...
                if remote_scalar.get('self_play/episode_return'):
                    self_play_return = remote_scalar.get('self_play/episode_return')
    ...
    def update_weights(self, model, batch, optimizer, replay_buffer, scaler, step_count, target_model=None):
        ...
        obs_batch = obs_batch_raw[:, 0: n_stack * self.obs_shape]  # current observation
        obs_target_batch = obs_batch_raw[:, self.obs_shape:]       # observation of next steps
        ...
        with autocast():
            for step_i in range(unroll_steps):
                ...
                # consistency loss
                gt_next_states = model.do_representation(obs_target_batch[:, step_i * self.obs_shape:(step_i + n_stack) * self.obs_shape])
    ...
    def log_hist(self, logger, distribution_dict, step_count):
        # MLFlow doesn't support histograms directly, so we'll log summary statistics instead
        for key, hist in distribution_dict.items():
            if len(hist) > 0:  # Ensure there's data to compute statistics
                mlflow.log_metrics({
                    f"{key}/mean": float(np.mean(hist)),
                    f"{key}/std": float(np.std(hist)),
                    f"{key}/min": float(np.min(hist)),
                    f"{key}/max": float(np.max(hist)),
                    f"{key}/median": float(np.median(hist))
                }, step=step_count)
